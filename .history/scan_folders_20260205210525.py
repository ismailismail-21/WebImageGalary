...
def scan_folder(folder_path, dataset_path, with_thumbnails=False):
    ...
    for filename in files:
        ...
        try:
            ...
            # Generate thumbnail if requested, or find existing one
            thumbnail_path = None
            if with_thumbnails:
                ...
                for ext in ['.webp', '.jpg']:
                    possible_thumb = os.path.join(thumb_dir, f"{name}_thumb{ext}")
                    if os.path.exists(possible_thumb):
                        thumbnail_path = os.path.relpath(possible_thumb, dataset_path)
                        break
