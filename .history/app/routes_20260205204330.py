...
def get_thumbnail(folder_name, filename):
    ...
    try:
        from .utils import generate_thumbnail, is_video
        ...
        thumb_rel_path = generate_thumbnail(full_path, DATASET_PATH)
        
        if thumb_rel_path:
            thumb_full_path = os.path.join(DATASET_PATH, thumb_rel_path)
            if os.path.exists(thumb_full_path):
                # Determine mimetype based on thumbnail extension
                if thumb_rel_path.endswith('.webp'):
                    mimetype = 'image/webp'
                else:
                    mimetype = 'image/jpeg'
                return send_file(thumb_full_path, mimetype=mimetype)
        ...
        try:
            return send_file(os.path.join(DATASET_PATH, folder_name, filename))
        except:
