...
window.videoMuted = true; // Track video mute state across videos
...
function openLightbox(imgElement) {
    ...
    if (isVideo) {
        ...
        lightboxVideo.muted = window.videoMuted;
    ...
    // Fade in the image after lightbox is shown
    setTimeout(() => {
...
async function nextImage() {
    ...
    if (currentImageIndex >= allImages.length - 1) {
        ...
        if (typeof loadMoreImages === 'function' && typeof hasMoreImages !== 'undefined' && hasMoreImages) {
            // Load more images
            await loadMoreImages();
            // Move to next image (which should now be available)
            currentImageIndex = (currentImageIndex + 1) % document.querySelectorAll('.grid-item').length;
        } else {
            // No more images to load, wrap to start
            currentImageIndex = 0;
        }
    } else {
...
/**
 * Load current image to lightbox with smooth transition
 */
function loadImageToLightbox() {
    ...
    // Wait for fade-out transition (faster for smoother feel)
    setTimeout(() => {
        if (isVideo) {
            lightboxImage.style.display = 'none';
            lightboxVideo.style.display = 'block';
...
/**
 * Toggle zoom between 100% and 200% on double-click
 */
function toggleZoom() {
    const lightboxImage = document.getElementById('lightboxImage');
    if (!lightboxImage || lightboxImage.style.display === 'none') return;
...
// Double-click to zoom toggle (100% ↔ 200%)
document.addEventListener('dblclick', (e) => {
    ...
    if (isInLightbox) {
        ...
        // Check if double-click was on the image or video
