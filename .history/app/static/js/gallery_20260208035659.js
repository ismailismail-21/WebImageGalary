...
// Helper function to properly encode URL path components
// This handles special characters, spaces, and Unicode characters
...
function zoomImage(direction) {
    ...
    // Use adaptive zoom step: smaller steps for smoother zooming
    const zoomStep = currentScale < 2 ? 0.05 : 0.075;
    if (direction === 'in') {
...
function zoomImageAtPoint(direction, mouseX, mouseY, imgWidth, imgHeight) {
    ...
    if (newScale === 1) {
        ...
        // Calculate the point in image coordinates (relative to center)
        ...
        // New translation to keep the point under cursor
        // When we zoom, we need to adjust translation so the zoom point stays in place
        const newX = (currentX - pointX) * scaleFactor + pointX;
        const newY = (currentY - pointY) * scaleFactor + pointY;
...
function deleteLightboxImage() {
    ...
    fetch(buildApiUrl('/api/image', folderName, filename), {
        method: 'DELETE'
    })
        .then(r => r.json())
        .then(data => {
